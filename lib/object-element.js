...
module.exports = function ObjectElement(el) {
   ...
   * @return {Null}
   */
  objectElement.tie = function (callback) {
  ...
  objectElement.addEventListener = function (eventType, callback, capture) {
    this.el.addEventListener(eventType, function () {
      callback.apply(objectElement, arguments);
    }, capture);
  }

  objectElement.removeEventListener = function (eventType, callback, capture) {
    this.el.removeEventListener(eventType, function () {
      callback.apply(objectElement, arguments);
    }, capture);
  }

  objectElement.bind = function (eventType, callback, capture) {
    events.bind(this.el, eventType, function () {
      callback.apply(objectElement, arguments);
    }, capture);
  }

  objectElement.unbind = function (eventType, callback, capture) {
    events.unbind(this.el, eventType, function () {
      callback.apply(objectElement, arguments);
    }, capture);
  }

  objectElement.delegate = function (selector, eventType, callback, capture) {
    delegate.bind(this.el, selector, eventType, function (ev) {
      callback.apply(wraps(ev.target), arguments);
    }, capture);
  }

  objectElement.undelegate = function (selector, eventType, callback, capture) {
    delegate.unbind(this.el, eventType, function (ev) {
      callback.apply(wraps(ev.target), arguments);
    }, capture);
  }

  objectElement.on = function (eventname) {
    var selector, callback, capture;

    if (arguments.length === 2) {
      // .on('click', onClick)
      callback = arguments[1];
      proto.on(eventname, callback);
  ...
  objectElement.off = function (eventname) {
    var selector, callback, capture;

    if (arguments.length === 2) {
      // .off('click', onClick)
      callback = arguments[1];
      proto.off(eventname, callback);
